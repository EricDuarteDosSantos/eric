...
let alunos = [
  { id: 1, nome: "Eric Duarte dos Santos", cargo: "Programador", departamento: "Técnico de Informática" },
  { id: 2, nome: "Ruan Gonzaga Polovina", cargo: "Programador", departamento: "Técnico de Informática" },
  { id: 3, nome: "Leonardo", cargo: "Programador", departamento: "Técnico de Informática" },
];
...
app.get('/alunos/listar', authenticate, (req, res) => {
  res.json(alunos); // Responde com o array de servidores em formato JSON
});

// Rota para obter um servidor por ID (protegida por autenticação)
app.get('/alunos/listarAlunos/:id', authenticate, (req, res) => {
  ...
  const aluno = alunos.find(s => s.id === id); // Encontra o servidor com o ID especificado
  if (aluno) {
    res.json(aluno); // Responde com os dados do servidor encontrado
  } else {
    res.status(404).send('Alunos não encontrado'); // Responde com status 404 se o alunos não for encontrado
  }
});

// Rota para adicionar um novo servidor (protegida por autenticação)
app.post('/alunos/inserir', authenticate, (req, res) => {
  const novoAlunos = req.body; // Obtém o novo servidor do corpo da requisição
  novoAlunos.id = alunos.length ? alunos[alunos.length - 1].id + 1 : 1; // Define o ID do novo servidor
  alunos.push(novoAlunos); // Adiciona o novo servidor ao array
  res.status(201).json(novoAlunos); // Responde com status 201 e o novo servidor em formato JSON
});

// Rota para atualizar um servidor existente (protegida por autenticação)
app.put('/alunos/atualizar/:id', authenticate, (req, res) => {
  const id = parseInt(req.params.id); // Converte o parâmetro ID para um número inteiro
  console.log(`Recebida requisição PUT para ID: ${id} com dados: `, req.body); // Loga o ID e os dados recebidos
  const index = alunos.findIndex(s => s.id === id); // Encontra o índice do servidor com o ID especificado
  if (index !== -1) {
    alunos[index] = { ...alunos[index], ...req.body }; // Atualiza o servidor com os novos dados
    res.json(alunos[index]); // Responde com o servidor atualizado
  } else {
...
app.patch('/alunos/atualizar/:id', authenticate, (req, res) => {
  const id = parseInt(req.params.id); // Converte o parâmetro ID para um número inteiro
...
app.delete('/alunos/deletar/:id', authenticate, (req, res) => {
  const id = parseInt(req.params.id); // Converte o parâmetro ID para um número inteiro
  console.log(`Recebida requisição DELETE para ID: ${id}`); // Loga o ID recebido
  const index = alunos.findIndex(s => s.id === id); // Encontra o índice do servidor com o ID especificado
  if (index !== -1) {
    alunos.splice(index, 1); // Remove o servidor do array
